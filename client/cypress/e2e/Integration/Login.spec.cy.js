describe('Login', () => {
  describe('Mock login Calls', () => {
    it('Reset password', () => {
      cy.visit('/login?forgot=a&type=forgot&userId=1');
      cy.get('input[name="resetPassword"]').type('12345678');
      cy.get('input[name="resetConfirmPassword"]').type('12345678');
      cy.get('button[data-testid="reset"]').click();
    })

    it('Reset password mismatch', () => {
      cy.visit('/login?forgot=a&type=forgot&userId=1');
      cy.get('input[name="resetPassword"]').type('12345678');
      cy.get('input[name="resetConfirmPassword"]').type('123456789');
      cy.get('button[data-testid="reset"]').click();
    })
    ...
    it('Sign up verification', () => {
      cy.visit('/login?verify=a&type=newuser&userId=1');
    })
  })
